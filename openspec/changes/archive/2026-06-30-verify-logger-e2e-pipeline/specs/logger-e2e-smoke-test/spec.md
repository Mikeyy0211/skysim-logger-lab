...
Defines the step-by-step end-to-end smoke test for the Logger pipeline, covering Kafka topic creation, infrastructure startup, Keycloak token acquisition, SampleService checkout triggers, and authenticated API query verification. The smoke test verifies that HTTP request/response events from SampleService are published to Kafka, consumed by Logger.Api, and queryable via authenticated Logger.Api REST endpoints.

Also defines requirements for the Postman collection and environment files that provide a no-curl alternative for running the E2E demo.
